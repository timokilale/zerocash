                        ...
                        <select class="form-select @error('from_account_id') is-invalid @enderror"
                            ...
                            @foreach($accounts as $account)
                                <option value="{{ $account->id }}"
                                    ...
                                    @else
                                        {{ $account->user->name }} ({{ $account->accountType->name }})
                                    @endif
...
$(document).ready(function() {
    ...
    $('#verifyAccountBtn').click(function() {
        ...
        console.log('Starting account verification for:', accountNumber); // Debug log
        ...
        $.ajax({
            url: '{{ route("transfers.account-details") }}',
            method: 'GET',
            data: {
        ...
        .done(function(response) {
            console.log('Account verification response:', response); // Debug log
        ...
        .fail(function(xhr, status, error) {
            console.error('Account verification failed:', {xhr: xhr, status: status, error: error}); // Debug log
